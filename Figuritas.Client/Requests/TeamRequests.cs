    ...
    public class TeamHttpClient
    {
        ...
        public async Task<ApiResponse<List<Team>>> GetTeamsAsync()
        {
            try
            {
                ...
                return ApiResponse<List<Team>>.Fail(HttpExtensions.GetFriendlyErrorMessage(response.StatusCode, errorMsg));
            }
            catch (Exception)
            {
                return ApiResponse<List<Team>>.Fail(HttpExtensions.GetFriendlyConnectionError());
            }
        }
    }
}
