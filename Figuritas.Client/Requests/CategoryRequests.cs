    ...
    public class CategoryHttpClient
    {
        ...
        public async Task<ApiResponse<List<Category>>> GetCategoriesAsync()
        {
            try
            {
                ...
                return ApiResponse<List<Category>>.Fail($"Error del servidor: {response.StatusCode}. {errorMsg}");
            }
            catch (Exception ex)
            {
                return ApiResponse<List<Category>>.Fail($"Error de conexión: {ex.Message}");
            }
        }
    }
}
