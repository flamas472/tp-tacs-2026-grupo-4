    ...
    IUserRepository userRepo)
{
    ...
    public async Task<List<ExchangeSuggestionResponseDTO>> GetSuggestionsAsync(int callerId, int page, int pageSize)
    {
        ...
        var callerOwned = callerStickers
            .Select(us => us.Sticker.Id)
            .ToHashSet();

        var callerListings = callerStickers; // ya filtrado arriba
        ...
        var missingByUser = allCandidateMissing
            ...
            .ToDictionary(g => g.Key, g => g.Select(ms => ms.StickerId).ToHashSet());

        // Paso 6: construir sugerencias
        // Una sola query batch para obtener todos los usuarios candidatos
        var candidateUsers = _userRepo.GetByIds(candidateUserIds)
        ...
        foreach (var group in candidatesByUser)
        {
            ...
            var candidateMissing = missingByUser.GetValueOrDefault(candidateUserId, new HashSet<int>());
            candidateUsers.TryGetValue(candidateUserId, out var candidateUser);

            // Calcular una sola vez por candidato, antes del inner loop
            var isPerfectMatch = callerOwned.Any(sId => candidateMissing.Contains(sId));

            UserSticker? callerMatchListing = null;
            if (isPerfectMatch)
            {
                var matchedStickerId = callerOwned.First(sId => candidateMissing.Contains(sId));
                callerMatchListing = callerListings
                    .FirstOrDefault(us => us.Sticker.Id == matchedStickerId);
            }
            ...
            foreach (var listing in group)
            {
                results.Add(new ExchangeSuggestionResponseDTO
                {
                    ...
                    CallerUserStickerId = callerMatchListing?.Id,
                    CallerMatchedStickerNumber = callerMatchListing?.Sticker.Number,
                    CallerMatchedStickerDescription = callerMatchListing?.Sticker.Description
                });
            }
        }
        ...
        return results
            .OrderByDescending(s => s.IsPerfectMatch)
